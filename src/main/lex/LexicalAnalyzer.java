...
public class LexicalAnalyzer {
    ...
    public void analyze(File file) {
        InterFile.readFile(file, (symbol, prev) -> {
            if (checkDelimiters(symbol, prev))
                return;
    ...
    private boolean checkDelimiters(char symbol, char prev) {
        ...
        switch (currentState) {
            case WORD:
                if(!Character.isLetter(symbol) && !Character.isDigit(symbol)
                        && symbol != '_' && symbol != '$' && (symbol != '.' || prev == '.')) currentState = State.ERROR;
            ...
            case NUMBER:
                if(!Character.isDigit(symbol)) {
                    ...
                    if((Character.isLetter(prev) && !"EeXx".contains(String.valueOf(prev))) && Character.isDigit(symbol)){
                        currentState = State.ERROR;
                    }
                }
                value.append(symbol);
